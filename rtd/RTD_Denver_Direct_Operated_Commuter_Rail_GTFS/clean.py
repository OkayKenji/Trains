...
df = pd.read_csv('./rtd/RTD_Denver_Direct_Operated_Commuter_Rail_GTFS/stops.txt')
...
df_sorted = pd.read_csv('./rtd/RTD_Denver_Direct_Operated_Commuter_Rail_GTFS/stopsSorted.txt')
final_df = final_df.set_index('stop_name').loc[df_sorted['stop_name']].reset_index()
final_df.to_csv('./rtd/RTD_Denver_Direct_Operated_Commuter_Rail_GTFS/stops.txt',index=False)